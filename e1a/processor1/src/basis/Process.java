    ...
    public void receiveMessage(Message message) throws RemoteException{
        synchronized (vector_clock){
            ...
            if (vector_clock.greaterEqual(message.vector_clock)){
                ...
                System.out.print(").");
                System.out.println(message + " is put in buffer");
                ...
                if (!pair.getKey().equals(id)) {
                    ...
                    try {
                        ...
                        if (pair.getKey() == 3){
                            ...
                            //copy vectorclock
                            VectorClock vectorClockcopy = new VectorClock();
                            for (int key: message.vector_clock.getVector().keySet()){
                                vectorClockcopy.getVector().put(key, message.vector_clock.get(key));
                            }
                            Message messagecopy = new Message(message.sourece, message.destination, vectorClockcopy, content);
                            ...
                            timer.schedule(new TimerTask() {
                                public void run(){
                                    try{
                                        processes.get(messagecopy.destination).receiveMessage(messagecopy);
